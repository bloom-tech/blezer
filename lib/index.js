...
async function listen(pid, queues, dir) {
  ...
  const cont = true;
  while (cont) {
    const [, jid] = await redis.brpopAsync(...queues, 0);
    let task;

    try {
      const job = await redis.hgetallAsync(`blezer:jobs:${jid}`);
      ...
      task = TaskFactory.getInstance(job.task, jid);
      if (task.prePerform) {
        await task.prePerform(jobArgs);
      }
      await task.perform(jobArgs);
      if (task.postPerform) {
        await task.postPerform();
      }

      await redis.hsetnxAsync(`blezer:jobs:${jid}`, 'outcome', 'success');
    } catch (error) {
